    ...
    def update(self):
        ...
        for v in self.viruses:
                if not v.doesClear():
                    new.append(v)
        self.viruses = new
        popv = self.getTotalPop()
        popDensity = popv/float(self.maxPop)
        for i in range(popv):
                try:
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for time in range(numTrials):
        ...
        for k in range(300):
            pop[k] += float(patient.update())
    for i in range(300):
        pop[i] /= float(numTrials)
    pylab.plot(range(300),pop)
    pylab.title('SimpleVirus simulation')
    pylab.xlabel('Time Steps')
    pylab.ylabel('label: Average Virus Population')
    pylab.legend('a')
    pylab.show()
#
# PROBLEM 4
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for i in range(popv):
                try:
                    self.viruses.append(self.viruses[i].reproduce(popDensity,self.drugs))
                except NoChildException:
                    pass
        return self.getTotalPop()
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for time in range(numTrials):
        ...
        for k in range(steps):
            pop[k] += float(patient.update())
            rpop[k] += float(patient.getResistPop(['guttagonol']))
        patient.addPrescription('guttagonol')
        for k in range(steps,steps+150):
            pop[k] += float(patient.update())
            rpop[k] += float(patient.getResistPop(patient.getPrescriptions()))
    for i in range(steps+150):
        pop[i] /= float(numTrials)
        rpop[i] /= float(numTrials)
    pylab.plot(range(steps+150),pop)
    pylab.plot(range(steps+150),rpop)
